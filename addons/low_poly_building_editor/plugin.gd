...
func _forward_3d_gui_input(camera: Camera3D, event: InputEvent) -> int:
	...
	if m_tool_mode == MODE_RAIL:
		return _handle_rail_input(camera, event)
...
func _handle_rail_input(camera: Camera3D, event: InputEvent) -> int:
	...
	if event is InputEventMouseMotion:
		...
		if m_is_drawing_rail:
			...
			if mouse_motion.position.distance_to(m_rail_start_screen_position) >= WALL_DRAG_COMMIT_DISTANCE:
	...
	if (
		...
		if !m_rail_release_commits_preview:
			_set_status("Click another point to place the rail, or drag from the start point and release.")
			return _handled()
	...
	var coordinator := _get_or_create_coordinator(true)
	if coordinator == null:
		_set_status("Open or create a scene before drawing rails.")
		return _handled()
	...
	if !m_is_drawing_rail:
		...
		_set_status("Rail start captured. Drag and release, or click another point.")
		return _handled()
...
func _create_rail_preview(coordinator: Building3DScript) -> void:
	...
	_apply_debug_wireframe_to_node(m_rail_preview)
...
func _commit_rail(
	...
	undo_redo.add_do_method(self, "_do_add_node", coordinator, rail, scene_root, true)
	undo_redo.add_undo_method(self, "_undo_remove_node", coordinator, rail)
	undo_redo.commit_action()
	_set_status(
...
func _start_rail_drag(
	...
	rail.material_override = _build_preview_material(_rail_drag_color(endpoint, true))
	_select_node(rail)
	_set_status(
...
func _update_rail_drag(camera: Camera3D, mouse_position: Vector2) -> void:
	...
	rail.material_override = _build_preview_material(
		_rail_drag_color(m_drag_rail_endpoint, valid)
	)
	if valid:
		_set_status("Release to commit rail: %.2f units." % rail.get_rail_length())
	else:
		_set_status("Rail is too short.")
...
func _commit_rail_drag() -> void:
	...
	undo_redo.add_do_method(self, "_select_node", rail)
	undo_redo.add_undo_method(rail, "set_rail_points", old_start, old_end)
	undo_redo.commit_action()
	_reset_rail_drag_state()
	_set_status("Edited rail: %.2f units." % rail.get_rail_length())
...
func _cancel_rail_drag() -> void:
	...
	_reset_rail_drag_state()
	_set_status("Rail edit canceled.")
