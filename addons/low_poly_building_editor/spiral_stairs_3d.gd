...
func _append_layout_specific_segment_geometry(
	...
	_append_radial_wedge_sequence_primitive(
...
func _helical_tangent(theta: float, turn_sign: float) -> Vector2:
...
func _helical_rail_frame(rail: Dictionary, run_position: float) -> Dictionary:
	...
	var tangent_2d := _helical_tangent(theta, turn_sign)
	var point_2d := (
		Vector2(rail["center"])
		+ _radial_direction(theta, turn_sign) * float(rail["radius"])
	)
	if run_position < 0.0:
...
func _helical_rail_path_height(
	rail: Dictionary,
	run_position: float,
...
func _append_helical_rail_strip(
...
func _append_helical_rail_sweep(
	rail: Dictionary,
	minimum_run: float,
	maximum_run: float,
	bottom_height: float,
	...
	var samples := _helical_rail_sample_positions(
		rail,
		minimum_run,
		maximum_run,
	...
	for run_position in samples:
		...
		var slope := _helical_rail_path_slope(
			rail,
			run_position,
			lower_horizontal_end,
			upper_horizontal_start,
	...
	var start_tangent: Vector3 = _helical_rail_frame(rail, samples[0])["tangent"]
	var end_tangent: Vector3 = _helical_rail_frame(rail, samples[-1])["tangent"]
	_append_helical_rail_cap(
...
func _append_helical_rail_posts(
	rail: Dictionary,
	vertices: PackedVector3Array,
	normals: PackedVector3Array,
	colors: PackedColorArray,
...
func _append_helical_rail_primitive(
	...
	_append_helical_rail_posts(rail, vertices, normals, colors, indices)


func _append_radial_wedge_sequence_primitive(
	seg: Dictionary,
	vertices: PackedVector3Array,
	normals: PackedVector3Array,
	colors: PackedColorArray,
	...
	for tread_index in range(steps):
		...
		var leading_normal := -_helical_tangent(theta0, turn_sign)
		_append_segment_quad(
			seg, vertices, normals, colors, indices,
			Vector3(inner0.x, bottom, inner0.y),
			Vector3(outer0.x, bottom, outer0.y),
		...
		var trailing_normal := _helical_tangent(theta1, turn_sign)
		_append_segment_quad(
			seg, vertices, normals, colors, indices,
			Vector3(inner1.x, bottom, inner1.y),
			Vector3(outer1.x, bottom, outer1.y),
