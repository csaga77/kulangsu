...
- Selecting an authored `Wall3D`, `Floor3D`, `Stairs3D`, `Pillar3D`, `Roof3D`, or `BuildingOpening3D` node in the editor automatically switches the dock and 3D viewport toolbar to that block's editing mode. Door and window openings infer their mode from the stored opening variant.
...
- Solid Floor drawing snaps to `BuildingEditor3D.grid_step`, preserves the configured base Y height as the floor's top surface, and creates one persistent floor node per committed rectangle. Hole Floor drawing snaps to the hit floor's top-surface Y and appends one rectangular opening to that floor through undo/redo; intersecting holes merge into one rectilinear opening that preserves the combined outline.
...
- Walls intersecting visible roof render regions remove generated mesh and collision above the roof's bottom surface. The clip follows flat, shed, gable, hip, rotated, overhanging, and partially covered roof geometry; it is transient rebuild data and does not edit wall endpoints or extra segments.
- `BuildingEditor3D` watches authored wall and roof geometry in editor mode and refreshes transient wall/roof clipping when those source properties change outside the plugin commit path.
...
- The first solid floor click can create a `BuildingEditor3D` coordinator if the scene has none. Hole mode requires an existing floor target. Floor previews never mutate the scene or undo history.
- The first stairs click can create a `BuildingEditor3D` coordinator if the scene has none. Stairs previews never mutate the scene or undo history.
...
- `BuildingEditor3D` owns snapping, default wall settings, wall lookup, collinear merge target detection, geometry-only intersecting-wall clip refresh, and transient roof-to-wall underside clip refresh.
...
- Scripts: `addons/low_poly_building_editor/plugin.gd`, `building_editor_3d.gd`, `wall_3d.gd`, `floor_3d.gd`, `stairs_3d.gd`, `pillar_3d.gd`, `roof_3d.gd`, `building_opening_3d.gd`, `wall_segment_3d.gd`, `merged_wall_mesh_builder.gd`, `low_poly_building_editor_dock.gd`, `viewport_input_overlay.gd`, `viewport_input_capture.gd`
...
- If wall endpoint storage, floor corner or floor-hole storage, stairs footprint storage, pillar base/radius storage, roof footprint storage, opening child semantics, or intersection clipping behavior changes, update this file plus `contract.md`.

## Validation

- Headless smoke scene: `addons/low_poly_building_editor/test_low_poly_building_editor_3d.tscn` (covers mesh conventions, floor slab and floor-hole generation, stairs step generation, pillar style and rim generation, roof style generation, roof angle preservation, roof rotation, roof render-area merge detection, simplified polygon roof overlap clipping, mismatched roof overlap clipping, full-cover rejection, overhang-only roof clipping, roof-to-wall underside clipping, stale roof-cover refresh, opening rules, door base-edge allowance and double-panel generation, double-window pane generation, opening anchors following edited segments, snapping, merge detection including height-mismatch rejection, intersection detection, geometry-only wall instance clipping, intersection splitting for multi-segment wall geometry, manual joint insertion, joint endpoint movement across connected segments, endpoint detach/reconnect behavior, connected wall top-cap preservation, multi-segment geometry with correct top-cap area, collinear segment extension, opening placement on extra segments, junction-adjacent segment assignment, and junction-straddling rejection).
