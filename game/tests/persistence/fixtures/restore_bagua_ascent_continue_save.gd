...
func _restore_save() -> void:
	if !FileAccess.file_exists(BACKUP_PATH):
		print("No Milestone B autosave backup was present; nothing changed.")
		get_tree().quit(0)
		return

	var backup_file := FileAccess.open(BACKUP_PATH, FileAccess.READ)
	if backup_file == null:
		push_error("Could not read the Milestone B autosave backup.")
		get_tree().quit(1)
		return
	var backup_value: Variant = backup_file.get_var(false)
	if !(backup_value is Dictionary):
		push_error("The Milestone B autosave backup is malformed; it was left in place.")
		get_tree().quit(1)
		return
	...
	if remove_error != OK:
		...
		get_tree().quit(1)
		return
	print("Restored the story autosave that preceded the Milestone B fixture review.")
	get_tree().quit(0)
