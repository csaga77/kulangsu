...
func _run() -> void:
	...
	_assert_true(AppState.get_open_shortcuts().find("bi_shan_crossing") >= 0, "Bi Shan records the dependable route before saving")
	...
	_assert_true(AppState.get_open_shortcuts().find("bi_shan_crossing") >= 0, "Continue restores dependable route state")
