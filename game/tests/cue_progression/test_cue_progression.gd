...
func _run() -> void:
	...
	_assert_true(AppState.get_open_shortcuts().find("bi_shan_crossing") >= 0, "Bi Shan unlocks the cross-island shortcut reward")
	_assert_true(AppState.build_map_journal_text().contains("Bi Shan Cross-Island Route"), "The journal map lists the unlocked Bi Shan shortcut")
