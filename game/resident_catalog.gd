...
static func _ambient_residents() -> Dictionary:
	...
	var specs: Array[Dictionary] = [
		...
		{
			"id": "quiet_visitor_tao",
			"display_name": "Quiet Visitor Tao",
			"landmark": "Trinity Church",
			"role": "Stops by most afternoons simply to listen and leave before anyone notices.",
			"routine_note": "Usually near the rear church wall where outside sounds stay soft.",
			"melody_hint": "He notices when a place hums before anyone speaks.",
			"ambient_lines": [
				"I come here for the space between bells.",
				"The island is quieter than silence today. That is why everyone keeps looking up.",
			],
			"appearance": _look(
				"male",
				"taupe",
			...
			"spawn": _spawn("Trinity Church", Vector2(320.0, 180.0), 65.0, MOOD_SHAME),
		},
		{
			"id": "organ_tuner_rui",
			"display_name": "Organ Tuner Rui",
			"landmark": "Trinity Church",
			"role": "Checks old reeds and keeps the organ from drifting too far out of temper.",
			"routine_note": "Usually with a tuning fork near the church side door.",
			"melody_hint": "He can tell the missing phrase is older than the current festival arrangement.",
			"ambient_lines": [
				"The organ still wants to lead into something the rest of us forgot.",
				"If you hear a note that feels older than the room, keep it.",
			],
			"appearance": _look(
				"male",
				"olive",
			...
			"spawn": _spawn("Trinity Church", Vector2(80.0, 120.0), -70.0, MOOD_NORMAL),
		},
		{
			...
			"spawn": _spawn("Bi Shan Tunnel South", Vector2(-240.0, 170.0), -125.0, MOOD_NORMAL),
		},
		{
			...
			"spawn": _spawn("Bi Shan Tunnel South", Vector2(-90.0, 240.0), -85.0, MOOD_SMILE),
		},
		{
			"id": "shortcut_runner_bo",
			"display_name": "Shortcut Runner Bo",
			"landmark": "Bi Shan Tunnel",
			"role": "Carries notes and errands through the faster tunnel routes when they are behaving.",
			"routine_note": "Usually pacing where the tunnel path forks away from the main lane.",
			"melody_hint": "He notices which turns shave time and which only repeat the same phrase.",
			"ambient_lines": [
				"I can save ten minutes here unless the tunnel decides to loop the mood instead.",
				"Fast routes never sound proud. They sound certain.",
			],
			"appearance": _look(
				"male",
				"bronze",
			...
			"spawn": _spawn("Bi Shan Tunnel South", Vector2(60.0, 180.0), -45.0, MOOD_SMILE),
		},
		{
			...
			"spawn": _spawn("Bi Shan Tunnel South", Vector2(210.0, 120.0), 10.0, MOOD_SHAME),
		},
		{
			"id": "lantern_keeper_ping",
			"display_name": "Lantern Keeper Ping",
			"landmark": "Bi Shan Tunnel",
			"role": "Rotates fresh lanterns through the tunnel so people can trust the lit sections.",
			"routine_note": "Usually near a cluster of spare lantern hooks and oil tins.",
			"melody_hint": "He says the right echo always reaches the lamplight a moment early.",
			"ambient_lines": [
				"The lanterns brighten before a good route, almost as if they know.",
				"Bad echoes make light feel thin. Good ones make it feel patient.",
			],
			"appearance": _look(
				"male",
				"taupe",
			...
			"spawn": _spawn("Bi Shan Tunnel South", Vector2(-160.0, 60.0), -150.0, MOOD_NORMAL),
		},
		{
			"id": "note_collector_shi",
			"display_name": "Note Collector Shi",
			"landmark": "Bi Shan Tunnel",
			"role": "Keeps scraps of overheard tunes in a folded pocket notebook.",
			"routine_note": "Usually on the outer edge of the entrance where tunnel noise softens.",
			"melody_hint": "She thinks every tunnel route steals a different note from the same song.",
			"ambient_lines": [
				"I write melodies down before the tunnel edits them for me.",
				"The notes you lose here are usually the ones you needed to understand a place.",
			],
			"appearance": _look(
				"female",
				"brown",
			...
			"spawn": _spawn("Bi Shan Tunnel South", Vector2(130.0, 40.0), 55.0, MOOD_BLUSH),
		},
		{
