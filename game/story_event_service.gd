...
func build_context(subject_id: String = "", extra_context: Dictionary = {}) -> Dictionary:
	var context := {
		...
		"location": m_runtime.get_location(),
		"season_phase": m_runtime.get_season_phase(),
		"story_day": m_runtime.get_story_day(),
		"world_hour": m_runtime.get_world_hour(),
		"time_of_day": m_runtime.get_time_of_day(),
		"mode": m_runtime.get_mode(),
		"active_lead_id": m_runtime.get_active_lead_id(),
	}
...
func notify_world_event(event_id: String, payload: Dictionary = {}, context: Dictionary = {}) -> Dictionary:
	...
	if !normalized_event_id.is_empty():
		result["resolved"] = m_runtime.resolve_story_event(normalized_event_id)
...
func matches_conditions(conditions_value: Variant, context: Dictionary = {}) -> bool:
	...
	if expected_phase != null:
		if expected_phase is Array or expected_phase is PackedStringArray:
			var allowed_phases: Array[String] = _normalize_string_array(expected_phase)
			if allowed_phases.find(m_runtime.get_season_phase()) < 0:
				return false
		elif m_runtime.get_season_phase() != String(expected_phase):
			return false

	var expected_mode := String(conditions.get("mode", ""))
	if !expected_mode.is_empty() and m_runtime.get_mode() != expected_mode:
		return false

	var expected_chapter := String(conditions.get("chapter", ""))
	if !expected_chapter.is_empty() and m_runtime.get_chapter() != expected_chapter:
		return false
	...
	if expected_time_of_day != null:
		if expected_time_of_day is Array or expected_time_of_day is PackedStringArray:
			...
			for time_value in _normalize_string_array(expected_time_of_day):
				allowed_times.append(String(time_value).strip_edges().to_lower())
			if allowed_times.find(m_runtime.get_time_of_day()) < 0:
				return false
		elif m_runtime.get_time_of_day() != String(expected_time_of_day).strip_edges().to_lower():
			return false

	if conditions.has("story_day_min") and m_runtime.get_story_day() < int(conditions.get("story_day_min", 0)):
		return false

	if conditions.has("story_day_max") and m_runtime.get_story_day() > int(conditions.get("story_day_max", 0)):
		return false
	...
	if conditions.has("world_hour_min") or conditions.has("world_hour_max"):
		...
		if !m_runtime.is_world_hour_in_range(min_hour, max_hour):
			return false

	for flag_value in conditions.get("story_flag_all", []):
		if !bool(m_runtime.get_story_flag(String(flag_value), false)):
			return false
	...
	if required_routes is Dictionary:
		for route_id_value in required_routes.keys():
			...
			var current_state := String(m_runtime.get_route_progress(route_id).get("state", "idle"))
	...
	if minimum_scores is Dictionary:
		for route_id_value in minimum_scores.keys():
			var route_id := String(route_id_value)
			if int(m_runtime.get_route_progress(route_id).get("completion_score", 0)) < int(minimum_scores[route_id_value]):
				return false
	...
	if required_landmarks is Dictionary:
		for landmark_id_value in required_landmarks.keys():
			...
			if !_matches_expected_value(m_runtime.get_landmark_state(landmark_id), required_landmarks[landmark_id_value]):
				return false
	...
	if required_progress_entries is Dictionary:
		for landmark_id_value in required_progress_entries.keys():
			...
			for progress_key_value in progress_requirements.keys():
				...
				var current_entries: Array[String] = _normalize_string_array(progress.get(progress_key, []))
	...
	if conditions.has("fragments_found_min") and m_runtime.get_fragments_found() < int(conditions.get("fragments_found_min", 0)):
		return false
	...
	if conditions.has("endgame_active") and bool(m_runtime.get_endgame_state().get("active", false)) != bool(conditions.get("endgame_active", false)):
		return false
...
func apply_effects(payload: Dictionary, context: Dictionary = {}) -> void:
	...
	if landmark_states is Dictionary:
		for landmark_id in landmark_states.keys():
			m_runtime.advance_landmark_state(String(landmark_id), String(landmark_states[landmark_id]))
	...
	if landmark_progress_list_append is Dictionary:
		for landmark_id_value in landmark_progress_list_append.keys():
			...
			for progress_key_value in append_requirements.keys():
				var progress_key := String(progress_key_value)
				var existing_values: Array[String] = _normalize_string_array(patched_progress.get(progress_key, []))
				var appended_values: Array[String] = _normalize_string_array(append_requirements[progress_key_value])
				for value in appended_values:
					if existing_values.find(value) < 0:
						existing_values.append(value)
				patched_progress[progress_key] = existing_values
			m_runtime.set_landmark_progress(landmark_id, patched_progress)
	...
	if !landmark_reward.is_empty():
		...
		if !bool(reward_result.get("handled", false)):
			m_runtime.resolve_landmark(landmark_reward)
	...
	if landmark_audio_cue_request is Dictionary:
		...
		if !cue_id.is_empty() and !cue_landmark_id.is_empty() and !cue_trigger_id.is_empty():
			m_runtime.request_landmark_audio_cue(cue_id, cue_landmark_id, cue_trigger_id, cue_display_name)

	var explicit_melody_hint := String(formatted_payload.get("melody_hint_text", "")).strip_edges()
	if !explicit_melody_hint.is_empty():
		m_runtime.emit_melody_hint(explicit_melody_hint)
	...
	if melody_prompt_request is Dictionary and !melody_prompt_request.is_empty():
		m_runtime.emit_melody_prompt(melody_prompt_request.duplicate(true))
	...
	if melody_prompt_request_builder is Dictionary and !melody_prompt_request_builder.is_empty():
		...
		if !melody_id.is_empty() and !prompt_mode.is_empty():
			m_runtime.request_melody_prompt(melody_id, prompt_mode, completion_kind, request_overrides)
	...
	if beat_story_flags is Dictionary:
		for flag_id in beat_story_flags.keys():
			m_runtime.set_story_flag(String(flag_id), beat_story_flags[flag_id])

	var story_event := String(formatted_payload.get("story_event", ""))
	if !story_event.is_empty() and m_runtime.can_resolve_story_event(story_event):
		m_runtime.resolve_story_event(story_event)

	var journal_unlocked_value = formatted_payload.get("journal_unlocked", null)
	if journal_unlocked_value != null:
		m_runtime.set_journal_unlocked(bool(journal_unlocked_value))
	...
	if unlock_shortcut_value is String:
		...
		unlocked_shortcuts = _normalize_string_array(unlock_shortcut_value)
	for shortcut_id in unlocked_shortcuts:
		m_runtime.unlock_shortcut(shortcut_id)

	var pin_lead_id := String(formatted_payload.get("pin_lead_id", ""))
	if !pin_lead_id.is_empty():
		m_runtime.pin_story_lead(pin_lead_id)
	...
	if resident_routine_overrides is Dictionary:
		for resident_id in resident_routine_overrides.keys():
			...
			if override_value is Dictionary:
				m_runtime.set_resident_routine_override(String(resident_id), override_value)

	var clear_override_ids: Array[String] = _normalize_string_array(formatted_payload.get("clear_resident_routine_override_ids", []))
	for resident_id in clear_override_ids:
		m_runtime.clear_resident_routine_override(resident_id)
	...
	if resident_routine_variant is Dictionary:
		for resident_id in resident_routine_variant.keys():
			...
			if variant_value is Dictionary:
				m_runtime.set_resident_routine_override(String(resident_id), variant_value)
	...
	if !milestone_id.is_empty():
		...
		if milestone_context is Dictionary:
			...
			m_runtime.emit_story_milestone(milestone_id, emitted_context)
		else:
			m_runtime.emit_story_milestone(milestone_id, resolved_context)
	...
	if festival_performed_milestone:
		m_runtime.emit_story_milestone("festival_performed", {
			"fragments_found": m_runtime.get_fragments_found(),
			"helped_residents": m_runtime.count_helped_residents(),
		})
	...
	if !landmark_resolved_milestone.is_empty():
		m_runtime.emit_story_milestone("landmark_resolved", {
	...
	m_runtime.refresh_story_routes()
...
func _describe_npc_subject(subject_id: String, context: Dictionary) -> Dictionary:
	...
	return {
		...
		"display_name": m_runtime.get_resident_display_name(resident_id),
		"prompt": "Talk to %s" % m_runtime.get_resident_display_name(resident_id),
		"consumed": true,
...
func _activate_npc_subject(subject_id: String, context: Dictionary) -> Dictionary:
	...
	result["display_name"] = m_runtime.get_resident_display_name(resident_id)
...
func _activate_landmark_subject(
	...
	if !binding.is_empty():
		...
		description["status_text"] = m_runtime.get_save_status()
		description["line"] = m_runtime.get_save_status()
		description["text"] = m_runtime.get_save_status()
		return description
	...
	var consumed: bool = m_runtime.activate_legacy_landmark_trigger(
...
func _matches_expected_value(current_value: Variant, expected_value: Variant) -> bool:
	if expected_value is Array or expected_value is PackedStringArray:
		var allowed_values: Array[String] = _normalize_string_array(expected_value)
...
func _progress_value_count(value: Variant) -> int:
	if value is Array or value is PackedStringArray:
		return _normalize_string_array(value).size()
...
func _apply_melody_source_award(award: Dictionary) -> void:
	...
	if counts_as_fragment and is_new_source:
		var fragments_total := int(melody_state.get("fragments_total", m_runtime.get_fragments_total()))
	...
	m_runtime.emit_story_milestone("fragment_restored", {
		"melody_id": melody_id,
		"source_id": source_id,
		"total_found": new_count,
	})

	if new_count >= int(melody_state.get("fragments_total", m_runtime.get_fragments_total())):
		m_runtime.emit_story_milestone("festival_ready", {
...
func _sync_festival_stage_availability(notify_player: bool = false) -> void:
	...
	var melody_ready := bool(m_runtime.get_story_flag("melody_bagua_aligned", false))
	var spring_ready := bool(m_runtime.get_story_flag("spring_festival_resolved", false))
	if melody_ready and spring_ready:
		...
		m_runtime.set_melody_progress(next_progress)
		if notify_player:
			m_runtime.set_objective("Return to Piano Ferry and perform the restored melody at the festival stage.")
			m_runtime.set_save_status("Spring Festival is ready — the harbor stage can finally answer the restored melody.")
		return

	if stage_state == "available":
		m_runtime.advance_landmark_state("festival_stage", "locked")
