...
const LEVEL_AREA_SCRIPT := preload("res://common/level_area_2d.gd")
...
func _run() -> void:
	...
	var relative_area = LEVEL_AREA_SCRIPT.new()
	relative_area.name = "RelativeArea"
	relative_area.level_id_mode = LEVEL_REGISTRY.LevelIdMode.RELATIVE_TO_PARENT
	relative_area.level_id = 0
	relative_area.sync_z_index_to_resolved_level = true
	child_room.add_child(relative_area)

	var external_container := Node2D.new()
	external_container.name = "ExternalContainer"
	add_child(external_container)

	var context_area = LEVEL_AREA_SCRIPT.new()
	context_area.name = "ContextArea"
	context_area.level_id_mode = LEVEL_REGISTRY.LevelIdMode.RELATIVE_TO_PARENT
	context_area.level_context_path = NodePath("../../ParentLevel/ChildRoom")
	context_area.level_id = 0
	context_area.sync_z_index_to_resolved_level = true
	external_container.add_child(context_area)
	...
	_assert_equal("Relative area resolves its own level from the closest level-aware parent", relative_area.get_resolved_level_id(), 4)
	_assert_equal("Relative area syncs its z index from the resolved level", relative_area.z_index, 4)
	_assert_equal("Context area resolves its own level from an explicit level context path", context_area.get_resolved_level_id(), 4)
	_assert_equal("Context area syncs its z index from the explicit level context path", context_area.z_index, 4)
	...
	_assert_equal("Relative area updates its own resolved level after the parent changes", relative_area.get_resolved_level_id(), 2)
	_assert_equal("Relative area updates its z index after the parent changes", relative_area.z_index, 2)
	_assert_equal("Context area updates its own resolved level after the context parent changes", context_area.get_resolved_level_id(), 2)
	_assert_equal("Context area updates its z index after the context parent changes", context_area.z_index, 2)
