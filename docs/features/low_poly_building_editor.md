...
- `ProceduralRoof3D` stores opposite parent-local footprint corners in `start_point` and `end_point`; its node transform and low-poly mesh are rebuilt from those corners with style, roof face angle in degrees, thickness, overhang, Y rotation, and color.
...
- Roofs merge only when their eave plane, style, roof face angle in degrees, thickness, overhang, exact rotation, and color match; mismatched overlapping roofs stay as separate authored nodes. Matching overhangs participate in render-area clipping even when committed footprints only touch or are separated by less than the combined overhang.
