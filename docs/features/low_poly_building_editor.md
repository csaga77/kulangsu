...
- Roofs merge only when their eave plane, style, rise, thickness, overhang, rotation, and color match; mismatched overlapping roofs stay as separate authored nodes.
...
- `BuildingEditor3D` owns default roof settings, roof node creation, and compatible roof merge detection alongside wall, floor, and pillar defaults.
...
- Headless smoke scene: `scenes/tests/test_low_poly_building_editor_3d.tscn` (covers mesh conventions, floor slab generation, pillar style and rim generation, roof style generation, roof rotation, roof intersection merge detection, opening rules, door base-edge allowance and double-panel generation, double-window pane generation, opening anchors following edited segments, snapping, merge detection including height-mismatch rejection, intersection detection, intersection splitting into editable endpoints, manual joint insertion, joint endpoint movement across connected segments, endpoint detach/reconnect behavior, connected wall top-cap preservation, multi-segment merged geometry with correct top-cap area, collinear segment extension, opening placement on extra segments, junction-adjacent segment assignment, and junction-straddling rejection).
