...
Each landmark can have one performance point: a `StorySubject3D` placed at the canonical performance spot listed in `melody_catalog.gd`. The player presses R at that node when the melody is in `reconstructed` state and any story-side gating for that performance point is satisfied. The recognition prompt opens. On success, `AppState.complete_prompt_request(...)` advances the melody or landmark and fires the appropriate world response.
...
- The in-world activation point still lives on a `StorySubject3D` at Festival Stage.
