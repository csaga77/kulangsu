...
- The current jump is cosmetic: only the visual model rises, while the collision capsule remains planted. It cannot clear obstacles, gaps, or ledges.
...
- A physical traversal jump must define numeric obstacle, gap, landing, timing, air-control, camera, and recovery fixtures before any room or route depends on it.
- A ladder must own an explicit 3D climb path, top/bottom mount and dismount transforms, clearance checks, collision policy, animation context, and blocked-exit fallback. Ladder climbing is an authored locomotion mode, not unconstrained surface climbing.
- Physical traversal jumping and ladder climbing are required planned capabilities. Each must ship in at least one production-space slice before the target character-action milestone is complete.

## Traversal Safety

- Dangerous drops must remain behind authored collision while the game has no player recovery service.
- Safe jump-down routes cannot be required until a physical traversal jump and fall-recovery contract exist.
- Before an unguarded fall ships, the world must track a recent safe semantic anchor and restore the player there without damage, story-progress loss, or an unrelated shared-state reload.
- Portals and authored exit transforms must place the actor on clear, stable ground and preserve intended orientation.
- Future movable objects need a deterministic reset path if they leave their authored bounds or obstruct required traversal.
...
- Add focused traversal-jump and ladder fixtures before either capability becomes production-critical; each fixture must cover entry/takeoff, movement, blocked paths, exit/landing, camera behavior, and recovery.
...
- Unguarded dangerous drops before the recovery service exists.
