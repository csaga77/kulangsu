...
- `HumanBody3D` supports physical traversal jumping, safe-transform recovery,
  gravity, native wall and stair-slope movement, and capped dynamic-body pushing.
- The production Bagua building now owns the first optional physical ascent: a
  guarded jump lane, recovery volume, and two-way service ladder. Its ordinary
  route and story subject remain available.
...
- A physical traversal jump uses the accepted obstacle, gap, landing, timing,
  air-control, ceiling, and recovery contract in the actor feature.
- A ladder owns an explicit 3D climb path, top/bottom mount and dismount transforms,
  expanded clearance checks, collision policy, animation context, and blocked-exit
  fallback. Ladder climbing is authored locomotion, not surface climbing.
- Traversal jump and ladder are current through the optional Bagua proof; new
  production spaces must reuse or deliberately extend those contracts.

## Traversal Safety

- Dangerous drops require authored collision plus an authored recovery area or the
  actor's safe-transform recovery.
- Required jump-down routes must remain within the accepted traversal and recovery
  limits.
