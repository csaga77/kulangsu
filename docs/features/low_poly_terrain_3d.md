...
- `test_street_mask_generation.gd` covers STREET-cell extraction into one network, stable IDs across regeneration, a bent path, dedicated multi-road junction meshes, a diagonal mask line collapsing to a straight run, visible generated segment meshes, absence of the obsolete mask-derived `StreetMesh`, segment/junction corridor shaping, deterministic replacement, exact reuse, and real-island visible streets/stairs. The addon's `test_street_network_3d.tscn` owns five-arm, unequal-width, curve, topology, overpass, stair-run, migration, restore, and dirty-rebuild fixtures; legacy Street3D crossing coverage remains in `test_street_3d.tscn`.
