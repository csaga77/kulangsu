...
- Water waves are wind-aware through a decoupled API: `LowPolyTerrain3D.set_wind(wind_angle_degrees, normalized_strength)` (plus the inspector-exported `wind_angle_degrees` / `wind_strength`) retunes the cached water materials live without a rebuild. The terrain does not import or read `WeatherManager`; a caller maps weather wind (e.g. `wind_angle_degrees` and a 0..1-normalized `wind_strength`) onto `set_wind()`. `scenes/tests/test_low_poly_world_3d.gd` drives a gusting stand-in for this until the lane is cleared to consume real weather.
