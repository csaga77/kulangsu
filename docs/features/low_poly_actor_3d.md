...
- When `move_and_slide()` reports a blocking wall contact, stair/floor snapping must preserve the slid XZ position and suppress forward step-floor probes so diagonal input carries the actor along the wall instead of snapping it back into the original into-wall target or climbing through side blockers. `RigidBody3D` contacts are excluded from that wall classification; the actor applies a small movement-direction impulse to push dynamic bodies such as balls instead of treating them as static walls.
