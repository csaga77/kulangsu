...
- [`../../assets/characters/3d_cartoon_boy_figure.glb`](../../assets/characters/3d_cartoon_boy_figure.glb) is the premade low-poly character model (skinned, textured, ~2k triangles) with `idle`, `walk`, and `run` animation clips. It is the default actor visual.
...
- [`../../characters/tests/test_human_body_3d.tscn`](../../characters/tests/test_human_body_3d.tscn) is the focused smoke scene covering actor API parity, current-frame controller input, placement occupancy, step-up/step-down navigation, and character-model structure (instanced model, mesh, material, animation clips, grounding).
