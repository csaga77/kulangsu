...
- [ ] Place four `LandmarkTrigger` nodes in `bi_shan_tunnel.tscn`: `echo_a`, `echo_b`, `echo_c`, and `chamber`.
- [ ] For echo triggers: set `landmark_id = "bi_shan_tunnel"`, `collected_progress_key = "echoes_collected"`, `visible_in_states = [available, introduced, in_progress]`.
- [ ] For the chamber trigger: same `landmark_id`, `requires_collected = [echo_a, echo_b, echo_c]`, `collected_progress_key = "echoes_collected"`, `visible_in_states = [in_progress]`.
- [ ] Position each trigger node at the matching world location in the tunnel.
