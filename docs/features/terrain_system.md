...
- [`../../terrain/low_poly_terrain_3d.gd`](../../terrain/low_poly_terrain_3d.gd) owns the parallel low-poly 3D terrain prototype. It reuses this terrain profile and mask semantics, can optionally expand generation to the full grayscale heightmap source as a smooth low-poly surface, treats sampled heightmap cells at or below `water_height` as water, keeps underwater terrain visible as continuous seabed, and renders flat semi-transparent vertex-colored 3D water with a visual adjacent-land overlap, surface layer, and separate shoreline highlight bands, but it is not part of the runtime 2D overworld contract yet.
