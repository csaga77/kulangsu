...
- `BuildingEditor3D` owns grid snapping, default floor, stairs, pillar, roof, and wall settings, floor/stairs/pillar/roof/wall creation, wall lookup, collinear wall merge detection, same-eave roof overlap clipping, intersecting-wall detection for editor commits, and geometry-only intersecting-wall clip refresh.
...
- `Wall3D` stores one primary parent-local span in `start_point/end_point` plus authored extra spans as `WallSegment3D` entries in `extra_segments`; intersecting sibling wall spans are passed in as transient non-rendered clip segments and do not become authored spans.
...
- `BuildingEditor3D` is the scene-owned coordinator for a building assembly; it owns grid snapping, eight-way drawing constraints, default wall, floor, pillar, and roof settings, floor/pillar/roof/wall creation, wall lookup, collinear wall merge detection, geometry-only intersecting-wall clip refresh, and same-eave roof overlap clipping
- selecting authored `Wall3D`, `Floor3D`, `Pillar3D`, `Roof3D`, or `BuildingOpening3D` nodes in the editor updates the active building tool mode through the dock/toolbar path; generic prop scene nodes do not auto-select Prop mode because they currently have no persistent prop marker
- `Wall3D` stores parent-local `start_point` and `end_point` for its primary span plus typed `WallSegment3D` resources in `extra_segments` for authored extra spans and manually inserted joints; transient sibling clip segments are used only for generated mesh/collision clipping and do not merge wall instances. The node transform derives from the primary span, shared joint endpoint drags move every connected segment endpoint together, single endpoint drags can disconnect from or reconnect to a shared joint, zero-length endpoint drags can promote an extra span to primary when the old primary is deleted, and one low-poly mesh plus generated collision rebuilds from authored spans, sibling clip spans, and direct child openings
...
- non-collinear intersecting walls sharing a base plane remain separate authored `Wall3D` nodes: sibling spans are supplied as non-rendered clip segments, each wall renders only its own authored spans, buried side and cap geometry is removed in plan space, same-height cap ownership follows scene order, and undo/redo refreshes clip geometry without reparenting children or removing other wall nodes
