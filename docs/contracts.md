...
- `BuildingEditor3D` owns grid snapping, default floor, pillar, roof, and wall settings, floor/pillar/roof/wall creation, wall lookup, collinear wall merge detection, same-eave roof overlap clipping, intersecting-wall detection for editor commits, and geometry-only intersecting-wall clip refresh.
...
- if wall endpoint storage, floor corner storage, pillar base/radius storage, roof footprint storage, segment splitting, child reparenting, or intersection clipping behavior changes, update this file and [`features/low_poly_building_editor.md`](features/low_poly_building_editor.md).
