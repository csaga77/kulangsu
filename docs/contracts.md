...
- direct child window openings keep their segment-local position and face alignment when wall geometry changes, including endpoint, joint, absorbed-segment, and whole-wall drags.
...
- `BuildingOpening3D` is the persistent child node used for window holes; its `opening_width`, `opening_height`, and segment-local position define the opening rectangle consumed by the wall mesh compiler, mapped to the nearest wall segment and restored into that segment's updated frame when wall geometry changes
