...
- `HumanBody3D.max_step_height`, `HumanBody3D.floor_snap_distance`, and `HumanBody3D.grounding_speed` tune prototype 3D navigation over floor meshes, including GridMap stair treads; solid wall geometry must still block traversal instead of being bypassed by stair support, while preserving lateral `move_and_slide()` motion and suppressing forward step-floor probes so diagonal wall contact slides instead of snapping into the blocked target
