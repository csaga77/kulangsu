...
- typed route resources are now the canonical narrative gate source for route events; `StoryRouteGraph.can_resolve_story_event(...)` and `get_story_event_blockers(...)` are the shared availability surface consumed by resident dialogue and StoryEvent effect application
