...
- `ProceduralWall3D` stores one primary parent-local span in `start_point/end_point` plus authored extra spans as `WallSegment3D` entries in `extra_segments`; intersecting sibling wall spans are passed in as transient non-rendered clip segments and do not become authored spans.
...
- non-collinear wall crossings keep each `ProceduralWall3D` as a separate authored node; wall creation, edits, undo, redo, and scene load refresh sibling clip segments so each wall removes only buried geometry under intersecting same-plane walls.
...
- non-collinear intersecting walls sharing a base plane remain separate authored `ProceduralWall3D` nodes: sibling spans are supplied as non-rendered clip segments, each wall renders only its own authored spans, buried side and cap geometry is removed in plan space, same-height cap ownership follows scene order, and undo/redo refreshes clip geometry without reparenting children or removing other wall nodes
