...
- `BuildingEditor3D` owns grid snapping, default floor, pillar, roof, and wall settings, floor/pillar/roof/wall creation, wall lookup, collinear wall merge detection, compatible roof overlap detection, and intersecting-wall detection for editor commits.
...
- `ProceduralRoof3D` stores opposite parent-local footprint corners in `start_point/end_point`, derives its transform from the minimum X/Z eave corner plus `roof_rotation_degrees`, and rebuilds one styled low-poly roof mesh plus generated collision from style, roof face angle in degrees, thickness, overhang, rotation, color, and any covered render-area rectangles. Roof tool creation converts the first draw point and current draw point into opposite corners of the rotated base footprint before deriving overhang, generated height, ridge, or apex geometry. Roof tool edits move the body, resize edges/corners on the active roof grid in the roof's rotated local frame, rotate around the footprint center, preserve the existing eave-plane Y, keep the roof angle fixed so generated height is derived from the current style-specific roof run, keep compatible roofs as separate authored footprints while clipping only covered render geometry from later roofs, compare roof compatibility by angle degrees, include matching overhangs in clipping, reject fully covered new or edited roofs, recompute sibling roof covers, and commit through undo/redo.
...
- `BuildingEditor3D` is the scene-owned coordinator for a building assembly; it owns grid snapping, eight-way drawing constraints, default wall, floor, pillar, and roof settings, floor/pillar/roof/wall creation, wall lookup, collinear wall merge detection, and compatible roof overlap detection
...
- `ProceduralRoof3D` stores opposite parent-local footprint corners in `start_point/end_point`; the node transform derives from the minimum X/Z eave corner plus `roof_rotation_degrees`, and one styled low-poly roof mesh plus generated collision rebuilds from style, roof face angle in degrees, thickness, overhang, rotation, color, and covered render-area rectangles. Roof tool creation converts the first draw point and current draw point into opposite corners of the rotated base footprint before deriving overhang, generated height, ridge, or apex geometry. Roof tool edits move the body, resize edges/corners on the active roof grid in the roof's rotated local frame, rotate around the footprint center, preserve the existing eave-plane Y, keep the roof angle fixed so generated height is derived from the current style-specific roof run, reject zero-area and fully covered roofs, keep compatible roofs as separate authored footprints while clipping only covered render geometry from later roofs, compare roof compatibility by angle degrees, include matching overhangs in clipping, recompute sibling roof covers, and commit through undo/redo
