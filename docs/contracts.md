...
- [`../addons/low_poly_building_editor/wall_3d.gd`](../addons/low_poly_building_editor/wall_3d.gd)
- [`../addons/low_poly_building_editor/floor_3d.gd`](../addons/low_poly_building_editor/floor_3d.gd)
- [`../addons/low_poly_building_editor/stairs_3d.gd`](../addons/low_poly_building_editor/stairs_3d.gd)
- [`../addons/low_poly_building_editor/pillar_3d.gd`](../addons/low_poly_building_editor/pillar_3d.gd)
- [`../addons/low_poly_building_editor/roof_3d.gd`](../addons/low_poly_building_editor/roof_3d.gd)
- [`../addons/low_poly_building_editor/wall_segment_3d.gd`](../addons/low_poly_building_editor/wall_segment_3d.gd)

Current contract:

- `BuildingEditor3D` owns grid snapping, default floor, stairs, pillar, roof, and wall settings, floor/stairs/pillar/roof/wall creation, wall lookup, collinear wall merge detection, same-eave roof overlap clipping, roof-to-wall underside clipping, intersecting-wall detection for editor commits, and geometry-only intersecting-wall clip refresh.
...
- `Wall3D` stores one primary parent-local span in `start_point/end_point` plus authored extra spans as `WallSegment3D` entries in `extra_segments`; intersecting sibling wall spans and roof underside surfaces are passed in as transient non-rendered clip data and do not become authored spans.
- `Floor3D` stores opposite parent-local footprint corners in `start_point/end_point` plus local-space rectangular `floor_holes`, derives its transform from the minimum X/Z top corner, extends thickness downward from the configured top surface Y, and rebuilds one low-poly mesh plus generated concave collision that preserves internal openings. Solid Floor tool edits move the body or resize edges/corners on the active floor grid, preserve the existing top-surface Y, reject resizes that would push stored holes outside the slab, and commit through undo/redo; Hole Floor tool commits append interior rectangular holes through undo/redo and merges intersecting holes into a non-overlapping rectangular union that preserves the combined outline.
- `Stairs3D` stores opposite parent-local footprint corners in `start_point/end_point`, derives its transform from the minimum X/Z lower-entry corner plus `stair_rotation_degrees`, and rebuilds one stepped low-poly mesh plus generated concave collision from total height, step count, underside thickness, rotation, and color. Stairs tool creation converts the first draw point and current draw point into opposite corners of the rotated footprint. Stairs tool edits move the body, resize edges/corners on the active stairs grid in the stairs' rotated local frame, rotate around the footprint center, preserve the existing lower-entry Y, reject zero-area footprints, and commit through undo/redo.
- `Pillar3D` stores one parent-local base point plus lower radius, optional upper radius override, height, side count, style, upper/lower rim height and outset, and color; the node transform derives from the base point, one styled low-poly pillar mesh plus generated collision rebuilds from those properties, and Pillar tool edits move the body or resize radius on the active pillar grid while preserving base Y and committing through undo/redo.
- `Roof3D` stores opposite parent-local footprint corners in `start_point/end_point`, derives its transform from the minimum X/Z eave corner plus `roof_rotation_degrees`, and rebuilds one styled low-poly roof mesh plus generated collision from style, roof face angle in degrees, thickness, overhang, rotation, color, covered render-area bounds or simplified cover polygons, hip gable drop, and optional debug triangle wireframe settings. Roof tool creation converts the first draw point and current draw point into opposite corners of the rotated base footprint before deriving overhang, generated height, and style-specific top geometry. Simple hip roofs use a horizontal ridge along the longer footprint axis, two triangular short faces, two trapezoid long faces, and the same configured angle on all faces; square hip footprints degenerate to a centered apex. Positive hip gable drop clips the hip ridge ends downward from the peak and extends the ridge as needed to preserve the configured face angle, producing a half-hip or jerkinhead variant without changing roof style. Roof tool edits move the body, resize edges/corners on the active roof grid in the roof's rotated local frame, rotate around the footprint center, preserve the existing eave-plane Y, keep the roof angle fixed so generated height is derived from the current style-specific roof run, keep same-eave roofs as separate authored footprints while clipping only polygon render regions whose roof surface sits under another roof surface, include matching overhangs in clipping, keep face-level intersection edges straight instead of stair-stepped through tiny rectangle fragments, close clipped edges with fascia faces so roof thickness remains visible along cuts, provide visible underside surfaces that clip intersecting wall mesh/collision above the roof bottom, reject fully covered new or edited roofs, recompute sibling roof covers, and commit through undo/redo. When enabled, `debug_show_triangle_wireframe` creates a generated `RoofTriangleWireframe` line mesh from the final triangle index buffer.
- non-collinear wall crossings keep each `Wall3D` as a separate authored node; wall creation, edits, undo, redo, and scene load refresh sibling clip segments so each wall removes only buried geometry under intersecting same-plane walls.
...
- direct child window and door openings keep their segment-local position and face alignment when wall geometry changes, including endpoint, joint, extra-segment, and whole-wall drags.

Governance:

- keep building-editor scene mutations undoable through `EditorUndoRedoManager`.
- if wall endpoint storage, floor corner or floor-hole storage, stairs footprint storage, pillar base/radius storage, roof footprint storage, segment splitting, child reparenting, or intersection clipping behavior changes, update this file and [`features/low_poly_building_editor.md`](features/low_poly_building_editor.md).
...
- in editor mode, `BuildingEditor3D` watches authored wall and roof geometry signatures and refreshes transient clipping data when those source properties change outside the plugin commit path
- generated wall/floor/stairs/pillar/roof collision, floor-hole side geometry, and opening frame parts are rebuild artifacts identified by metadata; they are not author-facing source nodes
- placed wall, floor, stairs, pillar, roof, prop, and opening mutations must go through `EditorUndoRedoManager` so undo/redo restores scene hierarchy and rebuilt meshes together
...
- keep this contract and [`features/low_poly_building_editor.md`](features/low_poly_building_editor.md) in sync when endpoint storage, floor corner or floor-hole storage, stairs footprint storage, pillar base/radius storage, roof footprint storage, opening semantics, generated collision, merge behavior, or editor interaction changes
