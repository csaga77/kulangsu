...
- `BuildingEditor3D` owns grid snapping, default floor, pillar, roof, and wall settings, floor/pillar/roof/wall creation, wall lookup, collinear wall merge detection, same-eave roof overlap clipping, and intersecting-wall detection for editor commits.
...
- direct child window and door openings keep their segment-local position and face alignment when wall geometry changes, including endpoint, joint, absorbed-segment, and whole-wall drags.
...
- `BuildingEditor3D` is the scene-owned coordinator for a building assembly; it owns grid snapping, eight-way drawing constraints, default wall, floor, pillar, and roof settings, floor/pillar/roof/wall creation, wall lookup, collinear wall merge detection, and same-eave roof overlap clipping
