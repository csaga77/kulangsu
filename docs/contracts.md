...
- `ProceduralRoof3D` stores opposite parent-local footprint corners in `start_point/end_point`, derives its transform from the minimum X/Z eave corner plus `roof_rotation_degrees`, and rebuilds one styled low-poly roof mesh plus generated collision from style, roof face angle in degrees, thickness, overhang, rotation, color, covered render-area bounds or simplified cover polygons, and optional debug triangle wireframe settings. Roof tool creation converts the first draw point and current draw point into opposite corners of the rotated base footprint before deriving overhang, generated height, ridge, or apex geometry. Roof tool edits move the body, resize edges/corners on the active roof grid in the roof's rotated local frame, rotate around the footprint center, preserve the existing eave-plane Y, keep the roof angle fixed so generated height is derived from the current style-specific roof run, keep same-eave roofs as separate authored footprints while clipping only polygon render regions whose roof surface sits under another roof surface, include matching overhangs in clipping, keep face-level intersection edges straight instead of stair-stepped through tiny rectangle fragments, close clipped edges with fascia faces so roof thickness remains visible along cuts, reject fully covered new or edited roofs, recompute sibling roof covers, and commit through undo/redo. When enabled, `debug_show_triangle_wireframe` creates a generated `RoofTriangleWireframe` line mesh from the final triangle index buffer.
...
- `ProceduralRoof3D` stores opposite parent-local footprint corners in `start_point/end_point`; the node transform derives from the minimum X/Z eave corner plus `roof_rotation_degrees`, and one styled low-poly roof mesh plus generated collision rebuilds from style, roof face angle in degrees, thickness, overhang, rotation, color, covered render-area bounds or simplified cover polygons, and optional debug triangle wireframe settings. Roof tool creation converts the first draw point and current draw point into opposite corners of the rotated base footprint before deriving overhang, generated height, ridge, or apex geometry. Roof tool edits move the body, resize edges/corners on the active roof grid in the roof's rotated local frame, rotate around the footprint center, preserve the existing eave-plane Y, keep the roof angle fixed so generated height is derived from the current style-specific roof run, reject zero-area and fully covered roofs, keep same-eave roofs as separate authored footprints while clipping only polygon render regions whose roof surface sits under another roof surface, include matching overhangs in clipping, keep face-level intersection edges straight instead of stair-stepped through tiny rectangle fragments, close clipped edges with fascia faces so roof thickness remains visible along cuts, recompute sibling roof covers, and commit through undo/redo. When enabled, `debug_show_triangle_wireframe` creates a generated `RoofTriangleWireframe` line mesh from the final triangle index buffer.
