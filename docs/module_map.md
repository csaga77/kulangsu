...
- [`../scenes/game_world_3d.tscn`](../scenes/game_world_3d.tscn) / [`../scenes/game_world_3d.gd`](../scenes/game_world_3d.gd) - production low-poly 3D overworld composition root: builds `LowPolyTerrain3D`, `HumanBody3D`, an orthographic camera, five landmark anchors (three stylized building instances plus tunnel markers), consumes the shared projection for player/resident configuration, composes focused surface-follow and story-interaction components, owns shared BGM/landmark-cue audio and generated landmark collision, and issues semantic location/checkpoint commands
...
- [`../game/world/story_interaction_coordinator.gd`](../game/world/story_interaction_coordinator.gd) - scene-local world-subject coordinator that owns subject discovery below one world root, deterministic proximity selection, hint/presentation behavior, and controller-driven dispatch to `AppState.activate_story_subject(...)`
...
- [`../game/story_subject_3d.gd`](../game/story_subject_3d.gd) - production `Area3D` world-subject adapter: exposes a stable `subject_id`, resolves action/display/presence from the shared StoryEvent catalog/`AppState`, and is dispatched by `StoryInteractionCoordinator` through `AppState.activate_story_subject`
...
- [`../scenes/tests/test_building_tour_3d.tscn`](../scenes/tests/test_building_tour_3d.tscn) - generic playable building-tour harness with an exported `building_scene`, transform and player spawn, plus `HumanBody3D` driven by `PlayerController3D`, camera-relative movement, a level orbit/zoom camera that does not follow jump height, lighting, and ground collision; defaults to the generated low-poly Bagua Tower concept
