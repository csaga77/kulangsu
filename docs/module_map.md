...
- [`../addons/storyline_editor/`](../addons/storyline_editor) - Storyline editor plugin with three integrated panels. Enable via Project → Project Settings → Plugins.
  - **Route browser** (left dock) — lists all routes with resource-vs-.gd badges, shows per-route event trees with prerequisite relationships, surfaces project-wide validation warnings, and provides a "+ New" button to scaffold a `StorylineRouteResource` .tres file. Selecting an event opens that `StorylineEventResource` in the Inspector for editing, and double-clicking an event highlights it in the graph editor.
  ...
  - **Inspector plugin** — adds a validation-warning panel and a collapsible known-event-IDs reference picker (click to copy) to the Inspector whenever a `StorylineEventResource` or `StorylineRouteResource` is selected.
