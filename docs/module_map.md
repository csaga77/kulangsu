...
- [`../game/story_route_graph.gd`](../game/story_route_graph.gd) - projects modular storyline definitions into route progress, lead selection, canonical story-event availability/blocker checks, endgame trigger logic, and tone-tag assembly
...
- [`../game/storylines/routes/`](../game/storylines/routes) - Drop `.tres` files of type `StorylineRouteResource` here. `StorylineCatalog` loads these as the canonical source; legacy `*_storyline.gd` support remains only as a compatibility path for older routes that still need migration.
