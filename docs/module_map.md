...
- [`../addons/storyline_editor/`](../addons/storyline_editor) - Storyline editor plugin with three integrated panels. Enable via Project → Project Settings → Plugins.
  - **Route browser** (left dock) — shows one combined storyline tree whose top-level rows are routes with resource-vs-.gd badges, whose child rows are the route's events, and whose event children show prerequisite relationships. It surfaces project-wide validation warnings and provides a "+ New" button to scaffold a `StorylineRouteResource` .tres file. Selecting a route opens that `StorylineRouteResource` in the Inspector for editing, selecting an event opens that `StorylineEventResource`, and double-clicking an event highlights it in the graph editor.
  - **Storyline Graph** (bottom panel) — `GraphEdit`-based dependency view/editor showing all events as color-coded nodes with prerequisite edges. Nodes expose separate `All` and `Any` input slots so authors can wire hard versus optional prerequisite flags directly in the graph. Supports per-route filtering, zoom/pan, dependency editing by connecting/disconnecting edges, per-user layout persistence across editor restarts, a toolbar `Arrange` action that resets the visible graph back to automatic dependency layout, and graph-node selection that opens the backing `StorylineEventResource` in the Inspector for editing. Cross-route dependencies are visible in "All routes" mode, and selecting or editing a legacy route can auto-promote it into `game/storylines/routes/` before inspector or graph edits are saved.
  - **Inspector plugin** — adds a validation-warning panel to the Inspector and replaces raw `story_flags_all` / `story_flags_any` string-array editing on `StorylineEventResource` objects with a route-rooted prerequisite picker that mirrors the storyline browser.
