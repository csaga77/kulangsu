...
- [`../assets/characters/`](../assets/characters) - premade skinned, textured low-poly character models (~2k triangles each) sharing `idle`/`walk`/`run`/`dance`/`scared`/`wave_goodbye` clips: `boy.glb` is the default `HumanBody3D` visual, with `female.glb` and `male.glb` as interchangeable alternates
