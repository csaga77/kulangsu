...
- [`../game/story_route_graph.gd`](../game/story_route_graph.gd) - keeps an instance-local runtime cache of modular storyline definitions, then projects them into route progress, lead selection, canonical story-event availability/blocker checks, endgame trigger logic, and tone-tag assembly
...
- [`../game/storylines/routes/`](../game/storylines/routes) - Drop `.tres` files of type `StorylineRouteResource` here. `StorylineCatalog` loads these as the canonical source; runtime `StoryRouteGraph` instances cache the loaded route/event bundle, while editor panels rebuild from the catalog during authoring refreshes. Legacy `*_storyline.gd` support remains only as a compatibility path for older routes that still need migration.
