...
- [`../game/storylines/routes/`](../game/storylines/routes) - Drop `.tres` files of type `StorylineRouteResource` here. `StorylineCatalog` loads them as the canonical source and falls back to the `*_storyline.gd` modules for any route not yet migrated.
