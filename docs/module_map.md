...
- [`../terrain/low_poly_terrain_3d.gd`](../terrain/low_poly_terrain_3d.gd) - parallel low-poly 3D terrain prototype that samples the existing terrain mask/profile and optional full-source heightmap into smooth continuous land/seabed terrain, heightmap-level water areas, flat semi-transparent vertex-colored water with adjacent-land render overlap, a semi-transparent water surface layer, water shoreline highlights, mask-clipped shoreline side walls, street, building-footprint, optional heightmap elevation, and optional collision meshes
...
- [`../game/storylines/`](../game/storylines) - shared storyline loader plus typed route resources under `routes/`; add new `StorylineRouteResource` files there instead of editing the central route graph
...
- [`../characters/procedural_low_poly_character_rig.gd`](../characters/procedural_low_poly_character_rig.gd) - optional runtime-generated low-poly 3D character rig with `Skeleton3D`, flat-shaded vertex-color `BodySurface`, and head/hand `BoneAttachment3D` nodes
...
- [`../architecture/low_poly/`](../architecture/low_poly) - low-poly 3D landmark proxy/prototype pieces for the parallel 3D exploration lane, including house, church, tunnel, and tower placeholder silhouettes
...
- [`../addons/low_poly_building_editor/`](../addons/low_poly_building_editor) - native editor dock and 3D viewport tool for low-poly building authoring. It registers `BuildingEditor3D`, `ProceduralWall3D`, and `BuildingOpening3D`; supports grid-snapped eight-way wall drawing, collinear wall merging, automatic absorption of intersecting walls into one multi-segment `ProceduralWall3D` with combined clipped geometry, translucent prop/window previews, generated collision for wall raycasts, and split wall meshes around window-opening child nodes. The focused smoke scene is [`../scenes/tests/test_low_poly_building_editor_3d.tscn`](../scenes/tests/test_low_poly_building_editor_3d.tscn), and the feature spec is [`features/low_poly_building_editor.md`](features/low_poly_building_editor.md).
- [`../addons/mp3_to_ogg/`](../addons/mp3_to_ogg) - editor dock plugin that batch-converts MP3 files to OGG Vorbis via ffmpeg; registers through Godot's editor-managed `EditorDock` API so its remembered right-dock placement restores cleanly from startup, remembers last-used per-user dock settings, provides ffmpeg/source/target pickers, a quality slider, background conversion with progress feedback, selectable conversion log output, and copy support. Use this when adding shipped `.ogg` assets under `resources/audio/`; Godot expects those files to be real Vorbis streams, not Ogg FLAC with a misleading extension.
- [`../addons/storyline_editor/`](../addons/storyline_editor) - Storyline editor plugin with three integrated panels. Enable via Project → Project Settings → Plugins.
- **Route browser** (Scene/Import dock stack) — shows one combined storyline tree whose top-level rows are routes and whose child rows are the route's events. The plugin now registers this dock through Godot's editor-managed `EditorDock` API instead of the older `add_control_to_dock()` path so the editor can restore its remembered Scene/Import tab placement cleanly from startup. It surfaces project-wide validation warnings and provides `+ New` plus a selection-driven `Delete` action. Deleting a selected route prompts for confirmation and removes that route's authored resource files; deleting a selected event prompts for confirmation and removes the event from the canonical typed route resource. If the deleted route or event was open in the Inspector, the editor clears that stale selection. Selecting a route opens that `StorylineRouteResource` in the Inspector for editing, selecting an event opens that `StorylineEventResource`, and double-clicking an event highlights it in the graph editor.
  - **Storyline Graph** (bottom panel) — `GraphEdit`-based dependency view/editor showing all events as color-coded nodes with prerequisite edges. Nodes expose separate `All` and `Any` input slots so authors can wire hard versus optional prerequisite flags directly in the graph. Supports per-route filtering, zoom/pan, dependency editing by connecting/disconnecting edges, dragging either end of an existing connection to remove it, checked-in layout persistence across editor restarts backed by [`../game/storylines/storyline_graph_layout.cfg`](../game/storylines/storyline_graph_layout.cfg), a toolbar `Arrange` action that resets the visible graph back to automatic dependency layout, and graph-node selection that opens the backing `StorylineEventResource` in the Inspector for editing. The graph now stays in sync with structural route/event changes from the browser and route-event inspector panel, so creating or deleting routes and adding or removing events refresh the route filter and visible nodes without a manual graph refresh. Cross-route dependencies are visible in "All routes" mode, and graph edits always write back to the saved typed route resource for the target event.
...
- [`../game/storylines/resources/`](../game/storylines/resources) - Typed GDScript Resource classes for the storyline schema: `StorylineEndingToneRule`, `StorylineEventResource`, `StorylineRouteResource`. Each class has `@export` fields covering all runtime dict keys plus a `validate()` method for duplicate-id, empty-field, phase-window, and prerequisite checks.
...
- `LowPolyTerrain3D` for the parallel terrain-mask/heightmap-to-3D prototype, including optional full-heightmap source expansion, heightmap elevation sampling, water-level classification, continuous visible seabed generation, and smooth low-poly land-surface generation
...
- `ProceduralLowPolyCharacterRig` for the runtime-generated low-poly character skeleton, flat-shaded mesh, vertex colors, and attachment nodes
