...
- [`../terrain/low_poly_terrain_3d.gd`](../terrain/low_poly_terrain_3d.gd) - parallel low-poly 3D terrain prototype node that owns the "cell grid -> meshes" half plus surface-height queries: smooth continuous land/seabed terrain, heightmap-level water areas, flat semi-transparent vertex-colored water with adjacent-land render overlap, a semi-transparent water surface layer, water shoreline highlights, mask-clipped shoreline side walls, street, building-footprint, optional heightmap elevation, and optional collision meshes
...
- [`../addons/low_poly_building_editor/`](../addons/low_poly_building_editor) - native editor dock and 3D viewport tool for low-poly building authoring. It registers `BuildingEditor3D`, `Wall3D`, `Floor3D`, `Stairs3D`, `Pillar3D`, `Roof3D`, and `BuildingOpening3D`; supports grid-snapped eight-way wall drawing, rectangular floor drawing/editing plus interior floor-hole cutting, stepped stairs drawing/editing with rotation, styled rimmed low-poly pillar placement/editing, styled low-poly roof placement/editing with rotation and compatible render-area clipping including matching overhangs, roof-to-wall underside clipping, collinear wall merging, geometry-only clipping for intersecting wall instances without merging nodes, translucent prop/window/door previews, generated collision for wall, floor, stairs, pillar, and roof meshes, and split wall meshes around window/door-opening child nodes. The focused smoke scene is [`../scenes/tests/test_low_poly_building_editor_3d.tscn`](../scenes/tests/test_low_poly_building_editor_3d.tscn), and the feature spec is [`features/low_poly_building_editor.md`](features/low_poly_building_editor.md).
