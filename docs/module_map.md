...
- [`../scenes/game_world_3d.tscn`](../scenes/game_world_3d.tscn) / [`../scenes/game_world_3d.gd`](../scenes/game_world_3d.gd) - parallel low-poly 3D overworld runtime candidate (Phase D+ of [`plan/low_poly_3d_replacement.md`](plan/low_poly_3d_replacement.md)): builds `LowPolyTerrain3D`, `HumanBody3D`, an orthographic camera, five landmark anchors (three stylized building instances plus invisible tunnel markers), maps the shared player profile to male/female/boy GLBs, spawns the shared wandering resident roster, owns 3D `StorySubject3D` interaction dispatch, shared BGM/landmark-cue audio, generated landmark collision, and location/landmark/resume syncing into `AppState`; `main.gd`'s `USE_3D_OVERWORLD` toggle (default off) instances it in place of `game_main` without deleting the 2D overworld
...
- [`../terrain/low_poly_terrain_3d.gd`](../terrain/low_poly_terrain_3d.gd) - parallel low-poly 3D terrain prototype node that owns exports, lifecycle, image loading, materials, wind, style resolution, and the public surface-height queries; it configures a `LowPolyTerrainSampler` and a `LowPolyTerrainMeshBuilder` and wraps the built buffers in `MeshInstance3D`/collision children
...
- [`../terrain/low_poly_street_corridor_integrator.gd`](../terrain/low_poly_street_corridor_integrator.gd) - generic pre-mesh terrain modifier that consumes duck-typed Street3D world corridors, lowers the supporting terrain bed under their full cross-section, feathers the boundary, suppresses underlying terrain street/building overlays in the core, and skips water unless explicitly enabled
...
- [`../characters/human_body_3d.gd`](../characters/human_body_3d.gd) / [`../characters/human_body_3d.tscn`](../characters/human_body_3d.tscn) - parallel low-poly 3D actor adapter prototype that renders one integrated premade GLB character model (scaled, yaw-corrected, auto-grounded, with idle/walk/run animation), with optional debug draws, gravity, wall sliding, floor snap, front-riser stair traversal, tagged stair-side rejection, and capped dynamic-body pushing; not wired into the runtime 2D overworld yet
...
- [`../addons/universal_lpc/`](../addons/universal_lpc) - reusable Universal LPC 2D runtime renderer, prebuilt metadata manifest, metadata generation/source-audit tooling, and addon-owned docs; see its [`README.md`](../addons/universal_lpc/README.md) and [`docs/contract.md`](../addons/universal_lpc/docs/contract.md)
...
- [`../addons/universal_lpc/`](../addons/universal_lpc) - runtime Universal LPC 2D character renderer with development-time generation/audit tooling. See the addon's own [`README.md`](../addons/universal_lpc/README.md), [`docs/feature.md`](../addons/universal_lpc/docs/feature.md), and [`docs/contract.md`](../addons/universal_lpc/docs/contract.md).
...
- [`../scenes/tests/test_street_mask_generation.tscn`](../scenes/tests/test_street_mask_generation.tscn) - focused STREET-mask regression covering centerline extraction, bent multipoint generated street mesh, terrain corridor shaping, and replacement rather than accumulation on rebuild
...
- `HumanBody3D` for the parallel low-poly 3D actor adapter prototype that renders one integrated premade GLB character model with idle/walk/run animation and no procedural body or separate clothing/hair attachment layer
