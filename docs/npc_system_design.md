...
2. [`../scenes/game_world_3d.gd`](../scenes/game_world_3d.gd) assigns landmark anchors and asks [`../characters/resident_factory.gd`](../characters/resident_factory.gd) to spawn residents.
3. The factory creates a `HumanBody3D`, selects its integrated GLB through [`../characters/character_model_catalog_3d.gd`](../characters/character_model_catalog_3d.gd), attaches `ResidentController3D`, and registers an `npc:<resident_id>` `StorySubject3D`.
...
5. Shared story/resident services apply dialogue, trust, route effects, and save state. The world presents the resulting line in a camera-facing `SpeechBalloon3D` and pauses/faces the resident locally.
...
- `ResidentFactory` owns one-time runtime assembly and presentation setup.
